...
for(i in 1:length(sample.size)){
  ...
  for(j in 1:datasets){
    ...
    diag(cov.mat) <- 1
    X <- draw.d.variate.normal(mean.vec = rep(0,covs),no.row = n,d = covs,cov.mat = cov.mat)
    ...
    results <- aenet_cox_tuned(NN = 15, time, delta, zz)
    ...
    MSEs[j,] <- c(t(as.matrix(results$beta.raw - beta)) %*% cov(zz) %*% as.matrix(results$beta.raw - beta),
                  ...
                  t(as.matrix(results$beta.aenet - beta)) %*% cov(zz) %*% as.matrix(results$beta.aenet - beta)
    )
    ...
    MSEs_var[j,] <- c(t(as.matrix(results$beta.raw - beta)) %*% diag(summary(coxph(Surv(time,delta)~zz))$coefficients[,3]) %*% as.matrix(results$beta.raw - beta),
                      ...
                      t(as.matrix(results$beta.aenet - beta)) %*% diag(results$beta.sd.aenet) %*% as.matrix(results$beta.aenet - beta)
    )

    # correct 0's #
    Corrects[j,] <- c(sum(results$beta.raw[-c(1:10)] == 0)/20,
                      sum(results$beta.lasso[-c(1:10)] == 0)/20,
                      sum(results$beta.aenet[-c(1:10)] == 0)/20
    )
    # incorrect 0's #
    Incorrects[j,] <- c(sum(results$beta.raw[c(1:10)] == 0)/10,
                        sum(results$beta.lasso[c(1:10)] == 0)/10,
                        sum(results$beta.aenet[c(1:10)] == 0)/10
    )
    # CPE #
    # new random dataset #
    ...
    fit_aenet <- coxph(Surv(time,delta) ~ ., data = dat_test, init = results$beta.aenet,iter=0)
    ...
    CPEs[j,] <- c(
      ...
      as.numeric(phcpe(coxph(Surv(dat_test$time, dat_test$delta) ~predict(fit_aenet, newdata=as.data.frame(dat_test)))))
    )

  }
  out_results <- list("MSEs" = MSEs, "MSEs_var" = MSEs_var,"Corrects" = Corrects,
                      "Incorrects" = Incorrects, "CPEs" = CPEs)
  save(out_results, file = paste0("results/ENET_StrongModel_Corr_", n,"_", "10cens",".Rdata"))
